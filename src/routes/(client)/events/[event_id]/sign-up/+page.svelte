<script lang="ts">
	...
	const initUserData = () => {
		...
		if (!user) return;
		signUpData.full_name = user.telegram_data?.first_name
			? `${user.telegram_data.first_name} ${user.telegram_data.last_name ?? ""}`.trim()
			: "";
	...
	$: price = eventInfo ? (signUpData.equipment_type === "RENT" ? 200 : eventInfo.cost) : 0;
	...
	const submit = async () => {
		...
		try {
			loading = true;
			await API.post(`/v1/events/${eventInfo.id_event}/members`, payload);
		...
		<div use:fadeUp={{ delayStep: 300 }}>
			<div class="opacity-0 md:container md:mx-auto px-4 py-4 space-y-3">
					...
					<div class="flex items-start gap-3">
						...
						<div class="flex-1">
							...
							<div class="flex items-center gap-2 text-xs text-white/50">
								<span><i class="bi bi-calendar-event-fill"></i> {eventDate?.toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric' })}</span>
								<span>•</span>
                ...
                <a href="/" class="text-blue-400 underline">политикой конфиденциальности</a>
								{#if errors.accept_rules}<br><span class="text-xs text-red-500 mt-1">{errors.accept_rules}</span>{/if}
              </span>
						</label>
					</div>
