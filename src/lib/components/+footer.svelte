...
<footer>
	<div class="fixed flex justify-around bottom-0 left-0 right-0 bg-[#1a1a2e] border-t border-white/5 px-2 py-2 pb-4 z-[1000] items-center">
		{#each links as l}
			<button on:click={() => goto(l.href, {invalidateAll: true})} class:active={page.url.pathname === l.href || page.url.pathname.startsWith(l.href + '/')} class="flex flex-col w-fit items-center gap-1 text-white/50 active:scale-95 transition-all duration-300 cursor-pointer">
